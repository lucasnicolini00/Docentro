...
describe('timeSlots actions', () => {
  ...
  describe('getTimeSlotsForBooking', () => {
    it('should get time slots for booking', async () => {
      ...
      vi.mocked(timeSlotsService.getClinic).mockResolvedValue(mockClinic as any)
      vi.mocked(timeSlotsService.getScheduleForDay).mockResolvedValue(mockSchedule as any)

      const result = await getTimeSlotsForBooking('doctor-123', 'clinic-123', DayOfWeek.MONDAY)
    ...
    it('should return error if clinic not found', async () => {
      vi.mocked(validateAuth).mockResolvedValue(mockSession as any)
      vi.mocked(timeSlotsService.getDoctor).mockResolvedValue(mockDoctor as any)
    ...
    it('should return empty slots if no schedule for day', async () => {
      ...
      expect(result.data?.message).toBe('No hay horarios disponibles para este día')
    })
  })
  ...
  describe('getAvailabilityOverview', () => {
    ...
    it('should filter by clinic if provided', async () => {
      ...
      expect(timeSlotsService.getAvailabilityOverview).toHaveBeenCalledWith('doctor-123', 'clinic-123')
    })
  ...
  describe('getTimeSlotsForCalendarAction', () => {
    it('should get time slots for calendar', async () => {
      ...
      const result = await getTimeSlotsForCalendarAction('doctor-123', 'clinic-123')
