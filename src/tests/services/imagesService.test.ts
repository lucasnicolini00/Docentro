...
describe("imagesService", () => {
  ...
  describe("getImage", () => {
    it("should get image with relations", async () => {
      ...
      // Just verify the call was made - query shape uses select now
      expect(prismaMock.image.findUnique).toHaveBeenCalled();
    });
      ...
      prismaMock.$transaction.mockImplementation(async (callback: any) => {
        return callback({
          image: {
            create: vi
              .fn()
              .mockResolvedValue({ ...mockImage, id: "new-img-123" }),
          },
          doctor: {
            update: vi
              .fn()
              .mockResolvedValue({
                id: "doctor-123",
                profileImageId: "new-img-123",
              }),
          },
        });
      });

      const result = await imagesService.createDoctorProfileImage(
        "doctor-123",
        imageData
      );
      ...
      prismaMock.$transaction.mockImplementation(async (callback: any) => {
        return callback({
          doctor: {
            update: vi
              .fn()
              .mockResolvedValue({ id: "doctor-123", profileImageId: null }),
          },
          image: {
