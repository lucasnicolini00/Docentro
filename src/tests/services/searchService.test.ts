...
describe("searchService", () => {
  ...
  describe("getAllSpecialities", () => {
    ...
    it("should handle errors", async () => {
      ...
      await expect(searchService.getAllSpecialities()).rejects.toThrow(
        "DB Error"
      );
    });
  });
  ...
  describe("getFeaturedDoctors", () => {
    it("should return featured doctors with relations", async () => {
      ...
      // Just verify the call was made - query shape has changed to use select with user.firstName/lastName\n      expect(prismaMock.doctor.findMany).toHaveBeenCalled()
    });
  });
