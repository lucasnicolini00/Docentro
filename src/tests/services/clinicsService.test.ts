...
describe('clinicsService', () => {
  ...
  describe('getDoctorClinics', () => {
    ...
    it('should throw error if doctor not found', async () => {
      ...
      await expect(clinicsService.getDoctorClinics('invalid-user')).rejects.toThrow('Doctor not found')
    })
  })
  ...
  describe('createClinic', () => {
    ...
    it('should throw error if doctor not found', async () => {
      ...
      await expect(clinicsService.createClinic('invalid-user', {} as any)).rejects.toThrow('Doctor not found')
    })
  })
  ...
  describe('updateClinic', () => {
    it('should update clinic if doctor has access', async () => {
      ...
      prismaMock.clinic.update.mockResolvedValue({ ...mockClinic, ...updateData } as any)

      const result = await clinicsService.updateClinic('user-123', 'clinic-123', updateData)
  ...
  describe('createPricing', () => {
    it('should create pricing if doctor has access to clinic', async () => {
      ...
      const result = await clinicsService.createPricing('user-123', pricingData)
    ...
    it('should throw error if doctor has no access to clinic', async () => {
      ...
      await expect(
        clinicsService.createPricing('user-123', { clinicId: 'clinic-123' } as any)
      ).rejects.toThrow('No tienes acceso a esta clínica')
    })
  })
  ...
  describe('updatePricing', () => {
    it('should update pricing if doctor owns it', async () => {
      ...
      const result = await clinicsService.updatePricing('user-123', 'pricing-1', updateData)
  ...
  describe('togglePricingStatus', () => {
    it('should toggle pricing status', async () => {
      ...
      const result = await clinicsService.togglePricingStatus('user-123', 'pricing-1')
  ...
  describe('deletePricing', () => {
    it('should soft delete pricing', async () => {
      ...
      const result = await clinicsService.deletePricing('user-123', 'pricing-1')
  ...
  describe('deleteClinic', () => {
    it('should soft delete clinic if no active appointments', async () => {
      ...
      const result = await clinicsService.deleteClinic('user-123', 'clinic-123')
    ...
    it('should throw error if clinic has active appointments', async () => {
      ...
      ).rejects.toThrow('No se puede eliminar la clínica porque tiene citas pendientes o confirmadas')
    })
