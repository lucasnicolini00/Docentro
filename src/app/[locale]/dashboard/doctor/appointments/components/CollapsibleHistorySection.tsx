...
export default function CollapsibleHistorySection({
  ...
  if (historicalAppointments.length === 0) return null;
        ...
        <div className="flex items-center space-x-3">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 text-left">
              {t("historyAppointmentsTitle")}
            </h3>
            <p className="text-sm text-gray-600 mt-1 text-left">
              {historicalAppointments.length} {t("historyPreviousCount")}
            </p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          <div className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
            {isHistoryExpanded ? t("hide") : t("viewHistory")}
          </div>
          {isHistoryExpanded ? (
