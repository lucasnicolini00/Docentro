...
export default function EnhancedScheduleManagement({
  ...
  const handleCreateSchedule = async (formData: FormData) => {
    ...
    const loadingToast = toast.loading(
      t("bulkCreatingSchedulesToast", { count: selectedDays.length })
    );
      ...
      try {
        ...
        if (successfulResults.length > 0) {
          ...
          if (failedCount === 0) {
            ...
            toast.success(
              t("bulkCreateSchedulesPartialSuccess", {
          ...
          toast.error(t("bulkCreateSchedulesError"), { id: loadingToast });
        }
      } catch (error) {
        // Clear optimistic schedules on error
        setOptimisticSchedules([]);
        console.error("Error creating schedules:", error);
        toast.error(t("bulkCreateSchedulesUnexpectedError"), {
          id: loadingToast,
        });
      }
    });
