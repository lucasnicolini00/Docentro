...
export default function ClinicsManagement({
      ...
      startTransition(async () => {
        try {
          ...
          if (result.success && result.data) {
            ...
            reject(new Error(result.error || t("confirmDeleteClinicError")));
          }
        } catch (error) {
          reject(error);
        }
  ...
  const handleDeleteClinic = async (clinicId: string) => {
    ...
    setConfirmAction({
      type: "clinic",
      id: clinicId,
      title: t("confirmDeleteClinicTitle"),
      message: t("confirmDeleteClinicMessage", { name: clinic?.name || "" }),
    });
  ...
  const handleCreatePricing = async (data: any) => {
    startTransition(async () => {
      ...
      if (result.success && result.data) {
        ...
        toast.success(t("confirmDeletePricingSuccess"));
  ...
  const handleDeletePricing = async (pricingId: string, clinicId: string) => {
    ...
    setConfirmAction({
      ...
      id: pricingId,
      clinicId,
      title: t("confirmDeletePricingTitle"),
      message: t("confirmDeletePricingMessage", {
        title: pricing?.title || "",
      }),
    });
    setShowConfirmModal(true);
    ...
    startTransition(async () => {
      if (confirmAction.type === "clinic") {
        ...
        if (result.success) {
          ...
          toast.success(t("confirmDeleteClinicSuccess"));
        } else {
          // Handle error and show toast notification
          toast.error(result.error || t("confirmDeleteClinicError"));
        ...
        if (result.success) {
          ...
          // Handle error and show toast notification
          toast.error(result.error || t("confirmDeletePricingError"));
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmationModal
        ...
        confirmText={t("deleteActionLabel")}
